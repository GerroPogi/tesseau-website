export async function onRequestPost(context) {
  ...
  console.log(
    `Received data: ${subject}, ${title}, ${deadline}, ${description}, ${type}, ${reference}, ${fileKeys}`
  );
  const row = await context.env.tesseau_db
    .prepare(
      "INSERT INTO reminders (subject, title, deadline, description, type, date_added, reference, file_key) VALUES (?,?, ?, ?, ?, ?, ?, ?)"
    )
    .bind(
      subject,
      title,
